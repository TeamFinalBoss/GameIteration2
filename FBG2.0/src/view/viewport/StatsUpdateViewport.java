...
public class StatsUpdateViewport implements ViewPort, Observer {
    ...
    public void draw(Graphics g) {
    	Healthable health = (Healthable) StatsMenu.getInstance();
    	options = health.getDescription();
    	currentHealth = health.getCurrentHealth();
        currentMana = health.getCurrentMana();
        maxHealth = health.getMaxHealth();
        maxMana = health.getMaxMana();
        yStart = GameDirector.getSize().height - (options.length * (height + padding));
        currentValue = health.getCurrentIndex() + 1;
        ...
        String healthe = currentHealth + "/" + maxHealth;
        g.setColor(Color.BLACK);
        g.drawString(healthe, healthBarWidth / 2 + healthBarXMargin - g.getFontMetrics().stringWidth(healthe) / 2, healthBarYMargin + g.getFontMetrics().getHeight());
        ...
        if (options != null) {
            ...
            for (int i = 0; i < options.length; i++) {
                ...
                if (strings.length == 2) {
                    ...
                    g.drawString(strings[1], screenWidth - width, yStart + i * (padding + height));
                } else {
